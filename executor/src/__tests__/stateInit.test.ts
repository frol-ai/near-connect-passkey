...
describe("deterministic passkey account ids", () => {
  ...
  // Full vectors generated by the Rust wallet crate for the deployed code ids
  // (see fixtures/wallet-vectors.json and fixtures.test.ts). Any change to the
  // State/StateInit layout or the code id breaks these on purpose.
  it("pins the full p256 vector", () => {
    expect(hex.encode(serializeDefaultStateInit(P256_KEY))).toBe(
      "0001" + // StateInit::V1, GlobalContractId::AccountId
        "2a000000" + // 42-char code id
        hex.encode(new TextEncoder().encode("0saf343be226341c0eca7dba6d0b29d49bdff3ad03")) +
        "01000000" + // data: 1 entry
        "00000000" + // key: STATE_KEY = ""
    ...
    expect(deriveAccountId(P256_KEY)).toBe("0seadc7532489fa16ec5f9e66e204689155c57acde");
  });

  it("pins the full ed25519 vector", () => {
    expect(deriveAccountId(ED25519_KEY)).toBe("0s86414e3c4592700ab237a6a70cdb5bba98179b3c");
  });
});
